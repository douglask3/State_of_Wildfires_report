...
dir = 'outputs/ConFire_Canada-biasC3/samples/crop_lightn_soilM_tree_biascorrected_csoil_pas_vpd_cveg_precip_tas_rhumid_totalVeg_biascorrected-frac_points_0.02/'


plot_control <- function(name, cols, levels, dats0 = NULL, addLab = FALSE, ...) {
    ...
    if (addLab) mtext(side = 3, '10%', adj = 0.15, line = -2)
    plotStandardMap(mx*100, cols = cols, limits = levels)
    if (addLab) mtext(side = 3, '90%', adj = 0.85, line = -2)
...
plot_controls <- function(controls, name) {
    png(paste0("r-code/ConFire_histMaps_fires-", name, ".png"), height = 4, width = 7, 
            res = 300, units = 'in')
        layout(rbind(c(1:3, 7:9), c(4:6, 10:12)), widths = c(1, 1, 0.5, 1, 1, 0.5))
        par(mar = c(0, 1, 0, 0), oma = c(0, 0, 0, 2))
        dats = plot_control(controls[1], cols, levels, extend_min = F, minLab = 0, addLab = T)
        lapply(controls[-1], plot_control, dcols, dlevels,dats, extend_min = T)
    dev.off()
}

plot_controls(c('factual-control', 'counterfactual-control', 'ss126_GFDL-control', 'ss585_GFDL-control'), "GFDL")
plot_controls(c('factual-control', 'counterfactual-control', 'ss126_IPSL-control', 'ss585_IPSL-control'), "IPSL")
plot_controls(c('factual-control', 'counterfactual-control', 'ss126_MPI-control' , 'ss585_MPI-control' ), "MPI")
plot_controls(c('factual-control', 'counterfactual-control', 'ss126_MRI-control' , 'ss585_MRI-control' ), "MRI")
