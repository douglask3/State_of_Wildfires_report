...
dir = paste0("outputs/ConFire_", region, "-nrt-tuning10/figs/_12-frac_points_0.5-baseline-control_TS/mean/")

files = list("points-Evaluate.csv", 
             "points-Control.csv",
...
cols = list(c('#ffffcc','#ffeda0','#fed976','#feb24c','#fd8d3c','#fc4e2a','#e31a1c','#bd0026','#800026', '#400013'),
            c('#ffffcc','#ffeda0','#fed976','#feb24c','#fd8d3c','#fc4e2a','#e31a1c','#bd0026','#800026', '#400013'),
...
dcols = list(rev(c('#35000f', '#67001f','#b2182b','#d6604d','#f4a582','#fddbc7','#f0f0f0','#d1e5f0','#92c5de','#4393c3','#2166ac','#053061', '#031531')),
             rev(c('#35000f', '#67001f','#b2182b','#d6604d','#f4a582','#fddbc7','#f0f0f0',
...
par(mfcol = c(7, 3), mar = c(0, 2, 0, 0), oma = c(0,2, 2, 3.5))
...
if (T) {
png(paste0("figs/control_stripes-JoeyStyle", region, ".png"), height = 7.5, width = 10, units = 'in', res = 300)

lmat = matrix(1:21, nrow = 7)
lmat = rbind(lmat[1,], c(22, 22, 23), lmat[-1,], c(24, 24, 25))
...
add_xaxis <- function() {
    axis(at = seq(15, 360-15, 30), labels = month.abb, side = 1, pos = par("usr")[3])
    axis(at = c(0, 360), labels = c('', ''), side = 1, pos = par("usr")[3], xpd = FALSE)
}
