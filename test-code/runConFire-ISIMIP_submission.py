...
input_dir = "isimip3a/driving_data/GSWP3-W5E5-20yrs/Global/"
output_dir = "isimip3a/outputs/test-alldat-noMT-DE/"
output_dir = "isimip3a/outputs/test-alldat-noMT-DE-ISIMIP_attribution/"
...
run_summery = np.any(np.array(output) == 'summery')

def makeSampleOutputFile(cube, varName, dir, period, ens_no):
    
    #var_dir = dir + '/' + varName + '/'
    #mkDir(var_dir)
    ens_dir = dir + 'ConFire_' + str(ens_no) + '_' + varName
    #mkDir(ens_dir)
    iris.save(cube, ens_dir + '_' + period + '.nc')
...
def runSample(paramLoc, output_dir, params, input_data, period):
    ...
    if ignore_lock_samples or not os.path.exists(lock_file):
        model = ConFire(input_data, params.loc[paramLoc], 
                        run_potential = run_potential, run_sensitivity = run_sensitivity,
        ...
        if run_Bootstraps: 
            ...
            for i in range(nboots):
                makeSampleOutputFileLocal(model.burnt_area_bootstraps[i], 
                                          'burnt_area_full_posterior', str(paramLoc)+ '-' + 
                                           str(i))
...
def run_for_period(period, experiment):
    ...
    output_dir_sample = output_dir + '/ensembles/'
    mkDir(output_dir_sample)
    output_dir_exp = output_dir_sample + '/' + experiment + '_'
    #mkDir(output_dir_exp)
    
    
    ngap =int(params.shape[0]/n_posterior_sample)
    sample_nos = range(0, params.shape[0], ngap)
    for i in sample_nos: runSample(i, output_dir_exp, params, input_data, period)
    ...
    #if run_summery and run_Bootstraps: summery_outputs("burnt_area_full_posterior")
    if run_summery and run_standard:
